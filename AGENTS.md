...
- `prod` is the production branch. Pushing it starts the GitHub Pages deployment
  for `https://bold-lab.ai`; publication waits for the existing environment
  reviewer approval.

Promote changes in one direction: `dev` to `staging`, then `staging` to `prod`.
Do not promote `dev` directly to `prod`.
