...
The website should embed the form inside the BOLD Opportunities page rather than sending applicants to a third-party form page. The MVP should use one shared Tally form with a hidden or pre-filled route field, so intake stays centralized without maintaining separate forms per route.

Issues #23 and #24 superseded the initial per-route Expression of Interest Page and route-specific conditional-section decisions: the website now owns Opportunity Route selection and route-specific context on the single `/opportunities` page, while the shared Tally form remains generic for the MVP.
